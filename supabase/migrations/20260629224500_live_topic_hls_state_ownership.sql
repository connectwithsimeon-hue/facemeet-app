-- Live Topic HLS state ownership fix.
-- Non-destructive: keeps Live Topic room start separate from HLS/Cloudflare start.
-- Only live_topic_hls_control should move HLS through pending/live/failed.
...
BEGIN
  ...
  SET status = 'live',
      ...
      ended_at = NULL,
      hls_status = CASE
        WHEN hls_status = 'pending' AND (hls_playback_url IS NULL OR trim(hls_playback_url) = '') THEN 'not_started'
        ELSE hls_status
      END,
      hls_started_at = CASE
        WHEN hls_status = 'pending' AND (hls_playback_url IS NULL OR trim(hls_playback_url) = '') THEN NULL
        ELSE hls_started_at
      END,
      hls_ended_at = NULL
...
BEGIN
  ...
  IF v_topic.status <> 'live' THEN
    RETURN jsonb_build_object(
      ...
      'hls_last_error_code', v_topic.hls_last_error_code
    );
  END IF;
...
DROP FUNCTION IF EXISTS public.list_live_now_topics();
CREATE FUNCTION public.list_live_now_topics()
RETURNS TABLE (
  id UUID,
...
CREATE FUNCTION public.get_live_topic_by_slug(p_slug TEXT)
RETURNS TABLE (
  id UUID,
  creator_user_id UUID,
  cohost_user_id UUID,
